...
class _CalendersScreensState extends State<CalendersScreens> {
  ...
  setDisabledDates() {
    ...
    setState(() {
      disableDates = cubit.selectedUnit.reservedDates!
      ...
      appBar: AppBar(
        ...
        title: Text(
          selectedDates.isEmpty
              ...
              : "${selectedDates.length} ${LocaleKeys.nights.tr()}",
            ...
            Expanded(
              ...
              child: PagedVerticalCalendar(
                dayBuilder: (context, day) => Padding(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                        ...
                        color: (disableDates.contains(day))
                            ? Colors.grey.withOpacity(0.5)
                            : selectedDates.contains(day)
                                ? ColorManager.mainlyBlueColor
                                : Colors.transparent,
                        border: Border.all(
                            color: selectedDisabledDates.contains(day)
                                ? ColorManager.mainlyBlueColor
                                : Colors.transparent)),
                    child: Center(
                        child: Padding(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                            color: (disableDates.contains(day) ||
                                    calculateDifference(day) < 0)
                                ? selectedDisabledDates.contains(day)
                                    ? ColorManager.mainlyBlueColor
                                    : Colors.grey
                                : selectedDates.contains(day)
                                    ? Colors.white
                                    : Colors.black,
                            decoration: disableDates.contains(day)
                ...
                onDayPressed: (selectedDateTime) {
                  DateTime clicked = DateTime(selectedDateTime.year,
                ...
                listener: (context, state) {
                  if (state is UpdateUnitSuccessAddDatesState) {
                    ...
                    Fluttertoast.showToast(
                      msg: LocaleKeys.failed_to_reserve_days.tr(),
                    ...
                    Fluttertoast.showToast(
                        msg: LocaleKeys.failed_to_free_days.tr(),
                        toastLength: Toast.LENGTH_SHORT,
                        gravity: ToastGravity.CENTER,
                        timeInSecForIosWeb: 1,
                        backgroundColor: ColorManager.darkGreyColor,
                        ...
                        onTap: selectedDates.isEmpty
                            ...
                            : () async {
                                List<String> dates = selectedDates
                                    .map((e) => e.toString())
                                    .toList();
                                cubit.updateUnitReservedDates(
                                    unitId: cubit.selectedUnit.id ?? -1,
                                    isAdd: true,
                                    dates: dates);
                              },
                        title: LocaleKeys.occupy.tr(),
                        clr: selectedDates.isEmpty
                            ? ColorManager.darkGreyColor
                            : ColorManager.mainlyBlueColor,
                        isLoading: state is UpdateUnitLoadingAddDatesState,
                      ),
                      KButton(
                        onTap: selectedDisabledDates.isEmpty
                            ? () {}
                            : () {
                                List<String> dates = selectedDisabledDates
                                    .map((e) => e.toString())
                                    .toList();
                                cubit.updateUnitReservedDates(
                                    unitId: cubit.selectedUnit.id ?? -1,
                        ...
                        title: LocaleKeys.make_available.tr(),
                        clr: selectedDisabledDates.isEmpty
                            ? ColorManager.darkGreyColor
                            : ColorManager.mainlyBlueColor,
                        isLoading: state is UpdateUnitLoadingRemoveDatesState,
                      ),
                    ],
                  );
                },
