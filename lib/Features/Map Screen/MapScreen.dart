...
class MapScreenState extends State<MapScreen> {
    ...
    return BlocConsumer<AppBloc, AppState>(
      ...
      builder: (context, state) {
        return Scaffold(
          ...
          floatingActionButton: !widget.showSelectLocation
              ? null
              : FloatingActionButton.extended(
                  ...
                  onPressed: () async {
                    ...
                    if (widget.isUpdate) {
                      await cubit.updateUnitMapLocation(
  ...
  void _moveMarker(double lat, double long) async {
    ...
    final Uint8List markerIcon =
        await getBytesFromAsset("assets/icons/marker.png", 150);
  ...
  Future<Position> _determinePosition() async {
    ...
    if (!serviceEnabled) {
      return Future.error(LocaleKeys.location_services_disabled.tr());
    }
    ...
    if (permission == LocationPermission.denied) {
      ...
      if (permission == LocationPermission.denied) {
        return Future.error(LocaleKeys.location_permissions_denied.tr());
      }
    }

    if (permission == LocationPermission.deniedForever) {
      return Future.error(
          LocaleKeys.location_permissions_permanently_denied.tr());
    }
