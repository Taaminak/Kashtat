...
class WalletRecordsScreen extends StatefulWidget {
  const WalletRecordsScreen({Key? key,}) : super(key: key);
  @override
...
class _WalletRecordsScreenState extends State<WalletRecordsScreen> {
  ...
  builder: (context, state) {
    ...
    return state is LoadingWalletState? Center(
      child: Loader(),
    ): SizedBox(
        width: size.width,
        height: size.height,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ContainerDecorated(content:  Row(
                      ...
                      children: [
                        Image.asset(ImageManager.credit, width: 32,color: Colors.black,),
                        SizedBox(width: 30,),
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            ...
                            // const SizedBox(height: 10),
                            Text('${cubit.walletLogs.isEmpty?0.0:cubit.walletLogs.first.user?.wallet??0.0} ر.س',style: TextStyle(
                                fontWeight: FontWeightManager.bold
                            ),),
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              SizedBox(height: 20,),
                             for(int i=0;i<(cubit.walletLogs.length);i++)
                             ContainerDecorated(content:  Row(
                               mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                               children: [
                                 FaIcon(cubit.walletLogs[i].amount!>0? FontAwesomeIcons.plusCircle :FontAwesomeIcons.minusCircle,color: cubit.walletLogs[i].amount!>0? ColorManager.greenColor : Colors.red ,size: 18,),
                                 Text('${cubit.walletLogs[i].amount} ر.س',style: TextStyle(
                                     fontWeight: FontWeightManager.medium,
                                   color: cubit.walletLogs[i].amount!>0? ColorManager.greenColor : Colors.red,
                                 ),),
                                 Text(DateFormat('yMMMMEEEEd').format(cubit.walletLogs[i].createdAt??DateTime.now()),style: TextStyle(
                                     fontWeight: FontWeightManager.medium,
                                   color:  cubit.walletLogs[i].amount!>0? ColorManager.greenColor : Colors.red,
                                 ),),
                               ],
