...
class AppBloc extends Cubit<AppState> {
  ...
  filterUsingCityId(int? cityId) {
    try {
      if (cityId == null) {
        ...
        List<UnitModel>? trips =
            allTrips?.data?.where((unit) => unit.city!.id == cityId).toList();
        homeTrips = homeTrips?.copyWith(data: trips ?? []);
      }
      emit(SuccessTripsState());
    } catch (e) {
  ...
  filterHomeUnitsByCategory({required int catId, int? subCatId}) {
    setHomeItems(homeTrips!.copyWith(
        data: allTrips!.data
            ?.where((trip) => subCatId != null
                ? /*trip.category!.id == catId && */ trip.subCategory!.id ==
                    subCatId
                : trip.category!.id == catId)
            .toList()));
    emit(SuccessTripsState());
  }
  ...
  void setReserveTripData({required UnitModel unit}) {
    ...
    requestTripModel = RequestTripModel(
      ...
      total: calculatePrice(selectedDates, unit.price ?? Price()).total +
          (calculatePrice(selectedDates, unit.price ?? Price()).total * 0.15),
  ...
  Future<void> reserveTrip(
    ...
    try {
      ...
      final result =
          await request.postRequest('/api/user/reservations', body, true);
      result.fold((failure) {
  ...
  Future<void> getUserTrips() async {
    ...
    result.fold((failure) {
      ...
      final decodedData = json.decode(result.data.toString());
      UserReservationModel? userTripsResponse =
          UserReservationModel.fromJson(decodedData);
  ...
  Future<void> rateReservation({required int rating}) async {
    ...
    try {
      final result = await request
          .postRequest(APIsManager.rateReservation, {"rating": rating});
      result.fold((failure) {
  ...
  Future<void> getAllProviderUnitsInOneLevel() async {
    ...
    result.fold((failure) {
      ...
      final decodedData = json.decode(result.data.toString());
      allProviderUnits = (decodedData['data'] as List)
  ...
  Future<void> getAllCategories({int? cityId}) async {
    ...
    try {
      ...
      result.fold((failure) {
        ...
        final decodedData = json.decode(result.data.toString());
        allCategories = List<CategoryModel>.from(decodedData['data']
  ...
  void setNewUnitCategoryList() {
    ...
    for (var e in allCategories) {
      if (e.subCategories!.isEmpty) {
        final sub = CreateNewUnitCategoryModel(
            name: e.name!,
            img: e.img!,
            serviceText: e.serviceName!,
            categoryId: e.id!);
  ...
  String getCategoryServiceName(int categoryId, int? subCategoryId) {
    String serviceName = '';
    try {
      serviceName = allCreateUnitCategory
          .firstWhere((category) =>
  ...
  void updateNewUnitCategoryAndTitle({
    ...
    updateUnitBody(
        unitBody.copyWith(categoryId: catId, subCategoryId: subCatId));
  }
  ...
  void updateNewUnitPricing({
    ...
    updateUnitBody(unitBody.copyWith(
        priceOthers: priceOfOthers,
        priceFriday: priceFriday,
        priceSaturday: priceSaturday,
        priceThursday: priceThursday));
  }
  ...
  void updateNewUnitTime({
    ...
    updateUnitBody(
        unitBody.copyWith(arrivalDate: arrival, leavingDate: leaving));
  }
  ...
  Future<void> updateNewUnitPhotos(List<XFile> images) async {
    ...
    for (var e in images) {
      var file = await MultipartFile.fromFile(e.path,
          filename: DateTime.now().toString());
      files.add(file);
    }
    otherPics = images;
  ...
  Future<void> createNewUnit() async {
    ...
    try {
      ...
      final result =
          await request.postRequest(APIsManager.createNewUnit, body, true);
      result.fold((failure) {
        throw Exception(failure.failure);
      }, (result) {
        clearUnitBody();
...
// ---------------- create new unit end ----------------
  ...
  Future<void> updateProfile({
    ...
    final result = await request.postRequest(
        APIsManager.updateProfile,
        FormData.fromMap({
          "phone": phone,
          "name": name,
          if (avatar != null)
  ...
  CategoryModel getCategoryById(int id) =>
      allCategories.firstWhere((element) => element.id == id);
      ...
      mapValue['data'].forEach((item) {
        item.forEach((key, value) {
          if (key == 'id') {
            ...
            providerCategoriesWithUnits.add(ProviderCategoriesWithUnits(
                name: key, units: units, categoryId: categoryId));
          }
        });
      });
    } catch (e) {
  ...
  Future<bool> updateUnit(
      ...
      bool isPostMethod = false}) async {
    // emit(UpdateUnitLoadingState());
    bool isSuccess = false;
    Either<Failure, Success> result;

    if (isPostMethod) {
      result = await request.postRequest(
          '${APIsManager.updateUnit}/$unitId', body, true);
    } else {
      result = await request.putRequest(
          '${APIsManager.updateUnit}/$unitId', body, true);
    }
  ...
  Future updateUnitStatus({required bool status, required int unitId}) async {
    ...
    await updateUnit(unitId: unitId, body: body).then((value) {
      if (value) {
        ...
        emit(UpdateUnitStatusFailureState(msg: 'Failed in change status.'));
      }
    });
  }
  ...
  Future updateUnitCityAndState({required int unitId}) async {
    ...
    emit(UpdateUnitLoadingState());
    await updateUnit(unitId: unitId, body: body)
        .then((value) => getAllProviderUnits());
  }
  ...
  Future updateUnitMapLocation({required int unitId}) async {
    ...
    await updateUnit(unitId: unitId, body: body).then((value) {
      if (value) {
        ...
        emit(UpdateUnitMapLocationFailureState(
            msg: 'Failed To update unit location.'));
      }
    });
  }
  ...
  Future updateUnitNameAndDetails({required int unitId}) async {
    ...
    await updateUnit(unitId: unitId, body: body).then((success) async {
      if (success) {
        ...
        emit(UpdateUnitNameAndDescFailureState(
            msg: 'Failed to update name and description'));
      }
    });
  }
  ...
  Future updateUnitPhotos({required int unitId}) async {
    ...
    emit(UpdateUnitLoadingState());
    await updateUnit(unitId: unitId, body: body, isPostMethod: true)
        .then((value) => getAllProviderUnits());
    clearUnitBody();
  }

  Future updateUnitReservedDates(
      {required int unitId,
      required bool isAdd,
  ...
  dynamic updateBodyData(UpdateType type) {
    if (type == UpdateType.status) {
      ...
      return {"state_id": unitBody.stateId};
    } else if (type == UpdateType.mapLocation) {
      return {"latitude": unitBody.latitude, "longitude": unitBody.longitude};
    } else if (type == UpdateType.nameAndDetails) {
      return {"name": unitBody.name, "description": unitBody.description};
      ...
      return {"reservations_dates": reservedDates};
  ...
  Future<bool> createCoupon({required FormData body}) async {
    ...
    try {
      ...
      final response =
          await dio.post('http://137.184.154.60/api/coupons', data: body);
  ...
  Future<int> validateCoupon(
      {required String code, required int unitId}) async {
    ...
    try {
      ...
      final result =
          await request.postRequest(APIsManager.validateCoupon, formData);
      result.fold((failure) {
        throw Exception(failure.failure);
      }, (result) {
        print(result.data);
  ...
  /// Financial Summary.
  Future<void> getFinancialSummary(
      {String? from, String? to, int? unitId, int? categoryId}) async {
    ...
    try {
      ...
      print(params);
      final result = await request
          .getRequest("${APIsManager.getFinancialSummary}?$params");
      result.fold((failure) {
        throw Exception(failure.failure);
      }, (result) {
        final data = json.decode(result.data.toString());
  ...
  Future<void> createNewBankAccount(
      {required String name, required String iban, required int bankId}) async {
    emit(CreateNewBankAccountLoadingState());
    try {
      final result = await request.postRequest(APIsManager.createNewBankAccount,
          {"name": name, "iban": iban, "bank_id": bankId});
      result.fold((failure) {
        throw Exception(failure.failure);
      }, (result) {
  ...
  Future<void> createNewComplain({required String message}) async {
    ...
    try {
      final result =
          await request.postRequest(APIsManager.createNewComplaints, {
        "message": message,
  ...
  Future<void> updateCategory(
      {required int categoryId, required Map<String, dynamic> body}) async {
    emit(UpdateCategoryLoadingState());
    try {
      final result = await request.postRequest(
          "${APIsManager.updateCategory}/$categoryId", body);
      result.fold((failure) {
        throw Exception(failure.failure);
      }, (result) {
  ...
  Future<void> getRatingSummary({int? unitId}) async {
    ...
    final result =
        await request.getRequest("${APIsManager.getRatingSummary}?$params");
    result.fold((failure) {
      rates = [];
      totalRate = 0;
...
enum ImagePickerType { singleImage, multiImages }
